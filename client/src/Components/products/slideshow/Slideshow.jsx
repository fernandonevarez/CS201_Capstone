...
const Slideshow = ({ items }) => {
  ...
  const end = (e) => {
    ...
    const dist =
      ((slider.move || e.changedTouches[0].clientX) -
      ...
      if (t >= 1) {
        const toid = setTimeout(() => {
          ...
          const idid = setInterval(() => {
            ...
            setScroll(
              dd > screenWidth / 2
                ? oo + (screenWidth - dd) * SLIDER_EASING_FUNCTION(tt)
                : oo - dd * SLIDER_EASING_FUNCTION(tt)
            );
  ...
  const move = (e) => {
    slider.move &&
      setScroll((s) => s + e.changedTouches[0].clientX - slider.move);
